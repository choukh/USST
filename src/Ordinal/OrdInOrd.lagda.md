...
前段是指序数 `α` 的底集 `⟨ α ⟩` 里小于某个元素 `a` 的那些元素, 它们具有类型 `B = Σ ⟨ α ⟩ (_≺ a)`, 且 `B` 也构成了一个序数, 记作 `α ↓ a`.
...
良基性的证明代码写起来相当简短, 但自然语言说起来比较绕. 我们要证任意前端元素 `(x , x ≺ a)` 在 `_≺′_` 关系下可及. 由良基归纳法, 有归纳假设: 对任意 `y ≺ x`, `(y , y ≺ a)` 可及. 要证 `(x , x ≺ a)`, 由可及的构造子 `acc`, 即证对任意 `(y , y ≺ a)`, 如果 `y ≺ x`, 那么 `(y , y ≺ a)` 可及. 由归纳假设即证. ∎
