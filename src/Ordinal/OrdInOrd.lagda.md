...
因为序数嵌入都是单射, 所以 `↑` 也是一个单射.

```agda
  ↑-inj : injective ↑
  ↑-inj = IsOrdEmbed.inj ↑-ordEmbed
```
...
**推论** 对序数取两次前段等价于/等于取一次较短前段.  
**证明** 分别由上两条引理结合 `↓≤` 即证. ∎

```agda
↓↓≃ : {a b : ⟨ α ⟩} {a≺b : a ≺⟨ α ⟩ b} → (α ↓ b) ↓ (a , a≺b) ≃ₒ α ↓ a
↓↓≃ = ↓≃ₒ↓ ↓≤ _

↓↓≡ : {a b : ⟨ α ⟩} {a≺b : a ≺⟨ α ⟩ b} → (α ↓ b) ↓ (a , a≺b) ≡ α ↓ a
↓↓≡ = ↓≡↓ ↓≤
```
...
严格序对序数等价有合同性, 这一性质用于将严格序的证明迁移到另一个宇宙, 会在下一章用到.

```agda
<-cong≃ₒ : α ≃ₒ β → γ ≃ₒ δ → α < γ → β < δ
<-cong≃ₒ {α} {β} {γ} {δ} α≃ₒβ γ≃ₒδ (x , γ↓x≡α) = y , ≃ₒ→≡ H
  where
  y = (γ≃ₒδ .fst ⁺¹) x
  H : (δ ↓ y) ≃ₒ β
  H = δ ↓ y ≃ₒ˘⟨ ↓≃ₒ↓ (≃ₒ→≤ γ≃ₒδ) x ⟩
...
**引理** 任意序数都不等价于其任意前段.  
**证明** 用归谬法, 假设有 `α ≃ₒ α ↓ a`. 由严格序的定义, 这等于说 `α < α`, 违反了严格序的反自反性. ∎

```agda
¬α≃ₒα↓a : (α : Ord 𝓊) (a : ⟨ α ⟩) → ¬ α ≃ₒ α ↓ a
¬α≃ₒα↓a α a α≃ₒα↓a = <-irrefl α (a , sym (≃ₒ→≡ α≃ₒα↓a))
```
...
**推论** `Ω` 不等价于其任意前段.  
**证明** 将 `¬α≃ₒα↓a` 实例化到 `Ω` 即得. ∎

```agda
¬Ω≃ₒΩ↓α : (α : Ord 𝓊) → ¬ Ω ≃ₒ Ω ↓ α
¬Ω≃ₒΩ↓α = ¬α≃ₒα↓a Ω
```
...
**定理** `Ω` 不与其中的任意 `α` 等价.  
**证明** 用归谬法, 假设存在 `α` 满足 `α ≃ₒ Ω`, 结合上一定理说的 `α ≃ₒ Ω ↓ α`, 由传递性有 `Ω ≃ₒ Ω ↓ α`, 与 `¬Ω≃ₒΩ↓α` 矛盾. ∎

```agda
Burali-Forti : ¬ (Σ α ∶ ⟨ Ω {𝓊} ⟩ , α ≃ₒ Ω)
Burali-Forti (α , α≃ₒΩ) = ¬Ω≃ₒΩ↓α α $
  Ω     ≃ₒ˘⟨ α≃ₒΩ ⟩
  α     ≃ₒ⟨ α≃Ω↓α ⟩
  Ω ↓ α ≃ₒ∎
```
